...
/**
 *     Update dpdx matrix of talc_mp
 *     */
void dpdx_mp_talc(void *SS_ref_db, const double *x){
    SS_ref *d  = (SS_ref *) SS_ref_db;
    double **dp_dx = d->dp_dx;

    dp_dx[0][0] = x[2] + x[1] - 1.0;      dp_dx[0][1] = x[0] - 2.0;      dp_dx[0][2] = x[0] - 2.0;      dp_dx[0][3] = 0.5*x[4] + 1.0;      dp_dx[0][4] = 0.5*x[3] - 0.5;      
    dp_dx[1][0] = 1.0 - 1.0*x[3];      dp_dx[1][1] = 0.0;      dp_dx[1][2] = 0.0;      dp_dx[1][3] = x[4] - 1.0*x[0];      dp_dx[1][4] = x[3] - 1.0;      
    dp_dx[2][0] = -1.0*x[2] + x[3] - 1.0*x[1];      dp_dx[2][1] = -1.0*x[0];      dp_dx[2][2] = -1.0*x[0];      dp_dx[2][3] = -1.5*x[4] + x[0];      dp_dx[2][4] = 1.5 - 1.5*x[3];      
    dp_dx[3][0] = 0.0;      dp_dx[3][1] = 2.00000000000000;      dp_dx[3][2] = 0.0;      dp_dx[3][3] = -2.00000000000000;      dp_dx[3][4] = 0.0;      
    dp_dx[4][0] = 0.0;      dp_dx[4][1] = 0.0;      dp_dx[4][2] = 2.00000000000000;      dp_dx[4][3] = 0.0;      dp_dx[4][4] = 0.0;      
    dp_dx[5][0] = 0.0;      dp_dx[5][1] = 0.0;      dp_dx[5][2] = 0.0;      dp_dx[5][3] = 1.00000000000000;      dp_dx[5][4] = 0.0;      
}
...
/**
 *     Endmember fraction of talc_mp
 *     */
void px_mp_talc(void *SS_ref_db, const double *x){
    SS_ref *d  = (SS_ref *) SS_ref_db;
    double *p = d->p;
        p[0]           = x[2]*x[0] - 2.0*x[2] + 0.5*x[4]*x[3] - 0.5*x[4] + x[3] + x[0]*x[1] - 1.0*x[0] - 2.0*x[1] + 1.0;
        p[1]           = x[4]*x[3] - 1.0*x[4] - 1.0*x[3]*x[0] + x[0];
        p[2]           = -1.0*x[2]*x[0] - 1.5*x[4]*x[3] + 1.5*x[4] + x[3]*x[0] - 1.0*x[0]*x[1];
        p[3]           = -2.0*x[3] + 2.0*x[1];
        p[4]           = 2.0*x[2];
        p[5]           = x[3];
}
...
double obj_mp_talc(unsigned n, const double *x, double *grad, void *SS_ref_db){
    ...
    sf[0]          = -1.0*x[4]*x[3] + x[4] + x[3]*x[0] - 1.0*x[3] - 1.0*x[0] + 1.0;
    sf[1]          = x[4]*x[3] - 1.0*x[4] - 1.0*x[3]*x[0] + x[0];
    sf[2]          = x[3];
    sf[3]          = x[2]*x[0] - 1.0*x[2] + 0.5*x[4]*x[3] - 0.5*x[4] + x[0]*x[1] - 1.0*x[0] - 1.0*x[1] + 1.0;
    sf[4]          = -1.0*x[2]*x[0] - 0.5*x[4]*x[3] + 0.5*x[4] - 1.0*x[0]*x[1] + x[0];
    sf[5]          = x[2];
    sf[6]          = x[1];
    sf[7]          = -1.0*x[2] + x[3] - 1.0*x[1] + 1.0;
    sf[8]          = x[2] - 1.0*x[3] + x[1];
    
    
    mu[0]          = R*T*creal(clog(sf[0]*cpow(sf[3], 2.0)*cpow(sf[7], 2.0))) + gb[0] + mu_Gex[0];
    mu[1]          = R*T*creal(clog(sf[1]*cpow(sf[4], 2.0)*cpow(sf[7], 2.0))) + gb[1] + mu_Gex[1];
    mu[2]          = R*T*creal(clog(cpow(sf[4], 2.0)*sf[0]*cpow(sf[7], 2.0))) + gb[2] + mu_Gex[2];
    mu[3]          = R*T*creal(clog(16.0*sf[6]*sf[8]*sf[0]*sf[3]*sf[7])) + gb[3] + mu_Gex[3];
    mu[4]          = R*T*creal(clog(16.0*sf[8]*sf[4]*sf[0]*sf[3]*sf[7])) + gb[4] + mu_Gex[4];
    mu[5]          = R*T*creal(clog(cpow(sf[6], 2.0)*cpow(sf[7], 2.0)*sf[2])) + gb[5] + mu_Gex[5];
...
/**
 *     Endmember to xeos for talc_mp
 *     */
void p2x_mp_talc(void *SS_ref_db, double eps){
    SS_ref *d  = (SS_ref *) SS_ref_db;
    
    d->iguess[4]  = d->p[0];
    d->iguess[3]  = d->p[5];
    d->iguess[2]  = 0.5*d->p[4];
    d->iguess[1]  = 0.5*d->p[3] + d->iguess[3];
    d->iguess[0]  = (3*d->p[1] + 2*d->p[2])/(1 + 2*d->p[1]+2*d->p[2]+d->p[0]+0.5*d->p[4]+0.5*d->p[3]-d->p[5]);
...
SS_ref PC_function(		global_variable 	 gv,
    ...
    if(gv.EM_database == 0){
		if 	(strcmp( name, "liq") == 0 ){
			...
			G0 = obj_mp_talc(SS_ref_db.n_xeos, SS_ref_db.iguess, SS_ref_db.dfx, &SS_ref_db);
		}
		else{
			printf("\nsolid solution '%s' is not in the database\n",name);		
		}	
